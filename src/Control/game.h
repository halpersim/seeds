...
namespace Control{
	...
	public:
			...
			sworm_metric()
		{
			...
			lists.tree.push_back(std::unique_ptr<GO::tree>(new GO::attacker_tree(create_hole(*lists.planet.back(), glm::vec2(2.6f, 0.f), 1.5f), *lists.planet.back())));
			lists.tree.push_back(std::unique_ptr<GO::tree>(new GO::defender_tree(create_hole(*lists.planet.back(), glm::vec2(0.f, 4.6462f), 1.5f), *lists.planet.back())));
			...
			lists.tree.push_back(std::unique_ptr<GO::tree>(new GO::attacker_tree(create_hole(*lists.planet.back(), glm::vec2(0.f), 1.5f), *lists.planet.back())));
			lists.tree.push_back(std::unique_ptr<GO::tree>(new GO::defender_tree(create_hole(*lists.planet.back(), glm::vec2(0.f, 2.f), 1.5f), *lists.planet.back())));
			...
			if(selected_id & DTO::id_generator::PLANET_BIT){
				...
				if(select_soldiers > 0){
					...
					for(auto it = lists.att.begin(); select_soldiers > 0 && it != lists.att.end(); it++){
						if(it->get()->get()->has_host_planet() && it->get()->get()->host_planet()->dto.ID == selected_id){
							it->get()->get()->dto->id = sworm_id;
							select_soldiers--;
						}
					}
		...
		inline void grow_tree(DTO::tree_type type){
			if((selected_id & DTO::id_generator::PLANET_BIT) &&
				...
				do{
					...
					it = std::find_if(lists.tree.begin(), lists.tree.end(), [planet, coords](const std::unique_ptr<GO::tree>& tree) {
				...
				for(std::shared_ptr<std::unique_ptr<GO::attacker>>& att : lists.att){
					if(attackers_ordered == Constants::DTO::ATTACKERS_REQUIRED_TO_BUILD_HOLE){
						break;
					}

					if(att->get()->has_host_planet() && att->get()->host_planet()->dto.ID == selected_id){
						att->reset(new GO::Attacker::ordered(*att->get(), *(att->get()->host_planet()), planet->dto.entry));
						attackers_ordered++;
					}
				}
			}
			...
			if(state.clicked.x >= 0){
				if(!hud_man.check_hud_clicked(state.clicked)){
					...
					if((selected_id & DTO::id_generator::SWORM_BIT) && (new_selected & DTO::id_generator::PLANET_BIT)){
						...
						std::for_each(lists.att.begin(), lists.att.end(), [this, new_selected, target_planet](std::shared_ptr<std::unique_ptr<GO::attacker>>& att_ptr){
							std::unique_ptr<GO::attacker>& att = *att_ptr;
							if(att->dto->id == selected_id && att->host_planet() != target_planet && (att->target_planet() != target_planet)) {
								att.reset(new GO::Attacker::moving(*att, *target_planet));
							}
						});
					} else{
		...
		inline void update(float d_time){
			if(!freeze){
				...
				std::for_each(lists.att.begin(), lists.att.end(), [this](std::shared_ptr<std::unique_ptr<GO::attacker>>& att) {
					if(att->get()->has_host_planet()){
						planet_state_tracker.add(*att->get()->host_planet(), att);
						planet_state_tracker.add_player_at_planet(*att->get()->host_planet(), att->get()->dto->owner.idx);
					}
				});

				std::for_each(lists.def.begin(), lists.def.end(), [this](std::shared_ptr<std::unique_ptr<GO::defender>>& def){
					planet_state_tracker.add(*def->get()->host_planet(), def);
					planet_state_tracker.add_player_at_planet(*def->get()->host_planet(), def->get()->dto->owner.idx);
				});

				std::for_each(lists.tree.begin(), lists.tree.end(), [this](std::unique_ptr<GO::tree>& tree){
					planet_state_tracker.add(tree->host_planet, tree->dto);
				});

				//iterate over all attacker 
				std::list<typename std::list<std::shared_ptr<std::unique_ptr<GO::attacker>>>::iterator> att_to_delte;

				for(auto it = lists.att.begin(); it != lists.att.end(); it++){
					std::unique_ptr<GO::attacker>& att = *(*it);

					//if its health drops below zero, it is dead and gets deleted
					if(att->dto->health < 0.01f){
						att_to_delte.push_back(it);
					} else {
						GO::attacker* new_state = att->update(time_elapsed);
						...
						if(att->has_host_planet() && att->get_state() != GO::attacker_state::FIGHTING && attacker_in_combat(att)){
							att.reset(new GO::Attacker::fighting(*att, *att->host_planet()));
						}
						...
						if(att->get_state() == GO::attacker_state::FIGHTING){
							...
							if(attacker_in_combat(att)) {
								if(att->dto->time_since_last_shot > Constants::Control::ATTACKER_ATTACKSPEED) {
									//if there is no enemy soldier on the planet anymore -> attack a tree
									if(planet_state_tracker.get_player_at_planet(*att->host_planet()).size() == 1){

									} else {
										std::weak_ptr<std::unique_ptr<GO::soldier>> target = std::weak_ptr<std::unique_ptr<GO::soldier>>();
										float min_distance = Constants::Control::SOLDIER_ENEMY_AWARENESS * 10;

										//find the closes target
										planet_state_tracker.for_each_soldier(*att->host_planet(), [&att, &target, &min_distance](std::shared_ptr<std::unique_ptr<GO::soldier>>& other){
										...
										//a target was found
										if(!target.expired()) {
											lists.bullets.push_back(GO::bullet(*att->host_planet(), target, att->get_coords(), att->dto->damage * Constants::Control::ATTACKER_BULLET_DAMAGE_FACTOR));
											att->dto->time_since_last_shot = 0.f;
										}
									}
								}
							} else {
								att.reset(new GO::Attacker::roaming(att->dto, *att->host_planet(), att->get_coords(), att->get_direction()));
							}
						}

						if(att->get_state() == GO::attacker_state::STUCK){							
							auto entry = reinterpret_cast<GO::Attacker::stuck*>(att.get())->entry_ptr.lock();
							...
							if(std::shared_ptr<DTO::planet_entry>& entry = att->host_planet()->dto.entry){
								if(glm::length(att->pos() - att->host_planet()->get_pos(entry->ground.coords, 0.f)) < Constants::DTO::DISTANCE_FLY_TO_PLANET_ENTRY &&
									...
									att.reset(new GO::Attacker::ordered(*(att.get()), *att->host_planet(), entry));
									break;
								}
							}
						}

						if(att->dto->id == selected_id){
						...
						} else if(att->dto->id != 0 && att->get_state() != GO::attacker_state::MOVING){
							att->dto->id = 0;
						}
					}
				}
				...
				std::list<typename std::list<std::shared_ptr<std::unique_ptr<GO::defender>>>::iterator> def_to_delete;
				
				for(auto it = lists.def.begin(); it != lists.def.end(); it++){
					std::unique_ptr<GO::defender>& def = *(*it);
					...
					if(def->dto->health < 0.f){
						...
						if(planet_state_tracker.get_player_at_planet(*def->host_planet()).size() > 1 && def->get_state() != GO::defender_state::FIGHTING){
							std::weak_ptr<std::unique_ptr<GO::soldier>> target_ptr = std::weak_ptr<std::unique_ptr<GO::soldier>>();
							...
							planet_state_tracker.for_each_soldier(*def->host_planet(), [&my_unit_coords, &other_coords, &def, &target_ptr, &min_time_to_impact](std::shared_ptr<std::unique_ptr<GO::soldier>>& other){
							...
							if(!target_ptr.expired()){
								if(auto target = target_ptr.lock()){
									glm::vec3 other_coords_delta_time = Control::GO::delta_coords_on_planet(glm::normalize(target->get()->get_direction()), target->get()->get_speed(), min_time_to_impact);
									...
									def.reset(new GO::Defender::fighting(def->dto, *def->host_planet(), target_ptr, my_utils::get_unit_coords(def->get_coords()), coords_at_impact, min_time_to_impact * 2));
								}
							}
						}
						...
						if(def->update(time_elapsed) && def->get_state() == GO::defender_state::FIGHTING){
							std::weak_ptr<std::unique_ptr<GO::soldier>>& target_ptr = reinterpret_cast<GO::Defender::fighting*>(def.get())->get_target();
							...
							def.reset(new GO::Defender::roaming(def->dto, *def->host_planet(), def->get_coords(), my_utils::get_random_dir()));
						}
					}
				}
				std::for_each(def_to_delete.begin(), def_to_delete.end(), [this](const auto& it){lists.def.erase(it); });

				//todo:den bledsinn ausihaun und oft schaun das no geht ######################################################################
				std::for_each(lists.def.begin(), lists.def.end(), [this](std::shared_ptr<std::unique_ptr<GO::defender>>& def_ptr) {
				...
				std::for_each(lists.tree.begin(), lists.tree.end(), [this](std::unique_ptr<GO::tree>& tree){
					if(tree->evolve(time_elapsed) && planet_state_tracker.num_soldiers(tree->host_planet) < tree->host_planet.dto.MAX_SOLDIERS){
						GO::soldier* new_soldier = tree->produce_solider();

						switch(tree->dto->TYPE){
							case DTO::tree_type::ATTACKER: lists.att.push_back(std::make_shared<std::unique_ptr<GO::attacker>>(std::unique_ptr<GO::attacker>(reinterpret_cast<GO::attacker*>(new_soldier)))); break;
							case DTO::tree_type::DEFENDER: lists.def.push_back(std::make_shared<std::unique_ptr<GO::defender>>(std::unique_ptr<GO::defender>(reinterpret_cast<GO::defender*>(new_soldier)))); break;
				...
				std::for_each(lists.planet.begin(), lists.planet.end(), [this](std::unique_ptr<GO::planet>& planet){
					if(planet->dto.entry && planet->dto.entry->stage >= Constants::DTO::ATTACKERS_REQUIRED_TO_FILL_HOLE){
						switch(planet->dto.entry->TREE_TYPE){
							case DTO::tree_type::ATTACKER: lists.tree.push_back(std::unique_ptr<GO::tree>(new GO::attacker_tree(planet->dto.entry->ground, *planet))); break;
							case DTO::tree_type::DEFENDER: lists.tree.push_back(std::unique_ptr<GO::tree>(new GO::defender_tree(planet->dto.entry->ground, *planet))); break;
		...
		private:
			...
			inline bool attacker_in_combat(const std::unique_ptr<GO::attacker>& att){
				...
				return planet_state_tracker.get_player_at_planet(*att->host_planet()).size() > 1 ||
					(att->dto->owner.idx != att->host_planet()->dto.owner.idx &&
