...
namespace Rendering{
	...
	public:
				...
				compute_program(false)
			{
				...
				fbo.add_color_texture(color_texture, 0);
				fbo.add_color_texture(id_texture, 1);
			...
			inline void render(
				const Rendering::_3D::free_cam& cam,
				...
				const std::list<DTO::planet<DTO::torus>>& planet_torus_list)
			{
				...
				int zero = 0;
				glClearBufferiv(GL_COLOR, 1, &zero);
				float one = 1.f;
				...
				compute_program.Uniform<border_size>() = 3;
				compute_program.Uniform<selected_id>() = 1;
				compute_program.use();
